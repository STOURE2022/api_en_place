"""
validator.py
Validation des données et qualité - VERSION CORRIGÉE
"""
...
class DataValidator:
    ...
    # ==================== ✅ NOUVEAU : VALIDATION FIX_PART ====================

    def validate_filename_pattern(self, fname: str, filename_pattern: str, 
                                   fix_part_pattern: str, table_name: str,
                                   log_quality_path: str) -> bool:
        """
        ✅ NOUVELLE FONCTION : Valide filename avec fix_part CASE-SENSITIVE
        
        Scénario 2 : Si fix_part ne match pas (case-sensitive) → REJECT
        
        Args:
            fname: Nom du fichier
            filename_pattern: Pattern complet (ex: "Billing_<yyyy><mm><dd>.csv")
            fix_part_pattern: Partie fixe attendue (ex: "Billing")
            table_name: Nom de la table
            log_quality_path: Chemin logs qualité
        ...
        # Exemple : "Billing_20251028.csv" → "Billing"
        parts = base.split('_')
        if not parts:
            return self._reject_filename(base, table_name, log_quality_path,
                                        "No fix_part found", "BBAFORGIF-102")
        
        fix_part_actual = parts[0]
        
        # ✅ Comparaison CASE-SENSITIVE
        if fix_part_actual != fix_part_pattern:
            return self._reject_filename(
                base, table_name, log_quality_path,
                f"Fix_part mismatch: expected '{fix_part_pattern}', got '{fix_part_actual}' (case-sensitive)",
                "BBAFORGIF-102"
            )
        
        print(f"✅ Fix_part validé : {fix_part_actual}")
        return True

    def _reject_filename(self, filename: str, table_name: str, log_quality_path: str,
                        error_msg: str, error_code: str) -> bool:
        """Helper pour rejeter un fichier"""
        print(f"❌ Rejeté : {filename} - {error_msg}")
        ...
        err_data = [(table_name, filename, "filename_pattern", error_msg, None, 1, error_code)]
